  ...
  return (
    <div className="guide-page min-h-dvh px-5 py-8 text-foreground sm:px-8 sm:py-12">
      <div className="guide-surface mx-auto max-w-3xl px-6 py-5 sm:px-10 sm:py-8">
        <Link
          href="/"
          className="inline-flex min-h-11 items-center gap-2 text-sm text-gold transition-colors hover:text-white focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-jade"
        >
          <ArrowLeft size={18} aria-hidden="true" />
          返回排盘
        </Link>
        <main id="main-content" className="pt-10 pb-16">
          <div className="mb-5 flex items-center gap-3 text-xs tracking-[0.25em] text-muted">
            <Compass size={18} aria-hidden="true" /> QIMEN · GUIDE
          </div>
          <h1 className="font-serif text-4xl leading-tight text-gold-light sm:text-5xl">
            使用指南
          </h1>
          ...
          <section
            className="mt-12 border-t border-gold-line pt-8"
            aria-labelledby="start-title"
          >
            <h2
              id="start-title"
              className="font-serif text-2xl text-gold-light"
            >
              输入与起局
            </h2>
            <ol className="mt-5 list-decimal space-y-3 pl-5 leading-8 text-foreground">
              <li>
            ...
            <p className="mt-5 rounded-xl border border-jade/20 bg-jade/5 p-4 text-sm leading-7 text-muted">
              支持公元 1–9999
          ...
          <section
            className="mt-10 border-t border-gold-line pt-8"
          ...
          >
            ...
            <dl className="mt-5 divide-y divide-gold-line">
              {annotations.map(([name, description]) => (
                <div
                  key={name}
                  className="grid gap-2 py-4 sm:grid-cols-[7rem_1fr]"
                >
                  <dt className="text-gold-light">{name}</dt>
          ...
          >
            ...
            <a
              className="mt-5 inline-flex min-h-11 items-center gap-2 text-sm text-gold underline decoration-gold/40 underline-offset-4 hover:text-white"
